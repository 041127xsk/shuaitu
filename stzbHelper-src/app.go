...
// SetDbDir 设置数据库存储目录
func (a *App) SetDbDir(dir string) string {
	if dir == "" {
		return global.Response{Message: "目录路径不能为空"}.Error()
	}
	// 检查目录是否存在，不存在则创建
	if err := os.MkdirAll(dir, 0755); err != nil {
		return global.Response{Message: "创建目录失败: " + err.Error()}.Error()
	}
	model.SetDbDir(dir)
	return global.Response{Message: "数据库目录设置成功: " + dir}.Success()
}

// GetDbDir 获取当前数据库存储目录
func (a *App) GetDbDir() string {
	...
	if dir == "" {
		...
		exePath, err := os.Executable()
		if err != nil {
			return global.Response{Data: ""}.Success()
		}
		dir = filepath.Dir(exePath)
	}
	return global.Response{Data: dir}.Success()
}
